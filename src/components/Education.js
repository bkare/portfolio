...
function Education({ education,labels }) {
  return (
    <section className="bg-white dark:bg-gray-800 p-4 rounded shadow mb-6 transition-colors duration-500">
      <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-blue-700 dark:text-blue-400 mb-4 transition-colors duration-400">{labels.education}</h3>
        ...
        {education.map((edu, i) => {
          ...
          return (
            <div key={i} className="border-l-4 border-blue-500 dark:border-blue-200 pl-4 transition-colors duration-400">
              <h4 className="text-lg font-bold text-blue-400 dark:text-blue-500 transition-colors duration-400">{school}</h4>
